...
function loadModels() {
  try { return JSON.parse(localStorage.getItem('ai_models') || '{}') } catch { return {} }
}
...
export default function App() {
  ...
  const [activeModels, setActiveModels] = useState(loadModels)
  ...
  const currentModel    = activeModels[activeProvider] || currentProvider.model

  const handleSaveSettings = (keys, provider, models) => {
    setApiKeys(keys)
    ...
    <div className="app-layout">
      <header className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button className="settings-btn" onClick={() => setShowSettings(true)}>⚙️ Sozlamalar</button>
        </div>
      </header>
      ...
      <div className="main-area">
        ...
        />
        <Preview
          resultHTML={resultHTML}   status={status}
          loading={loading}         formData={formData}
