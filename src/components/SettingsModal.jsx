...
export default function SettingsModal({ keys, activeProvider, activeModels, onSave, onClose }) {
  ...
  const setModel = (providerId, modelId) => setLocalModels(p => ({ ...p, [providerId]: modelId }))
  const toggleVisible = (id) => setVisible(p => ({ ...p, [id]: !p[id] }))

  const getSelectedModel = (p) => localModels[p.id] || p.model

  const handleSave = () => {
    onSave(localKeys, localProvider, localModels)
    onClose()
  }
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
            ...
            >
              <div className="provider-top">
                <div className="provider-left">
                  <span className="provider-radio">{localProvider === p.id ? '●' : '○'}</span>
              ...
              <div className="provider-key-row">
                ...
                />
                <button className="toggle-vis" onClick={e => { e.stopPropagation(); toggleVisible(p.id) }}>
                  {visible[p.id] ? '🙈' : '👁'}
                </button>
                <a href={p.keyHelp} target="_blank" rel="noreferrer" className="get-key-link" onClick={e => e.stopPropagation()}>
                  Kalit olish →
                </a>
              </div>

              {/* Model tanlash — Gemini va OpenRouter uchun */}
              {p.models && (
                <div className="model-selector" onClick={e => e.stopPropagation()}>
                  <span className="model-label">Model:</span>
                  <select
                    value={getSelectedModel(p)}
        ...
        <div className="tip-box">
          💡 <strong>Tavsiya:</strong> Google Gemini → <strong>Gemini 2.0 Flash Lite</strong> —
          to'liq bepul, karta kerak emas, tez ishlaydi.
          <a href="https://aistudio.google.com/apikey" target="_blank" rel="noreferrer"> Kalit olish →</a>
        </div>
